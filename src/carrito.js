...
let Shoppingcarrito = document.getElementById("shopping-carrito");
...
let generatecarritoItems = () => {
  if (basket.length !== 0) {
    return (Shoppingcarrito.innerHTML = basket
      .map((x) => {
        let { id, item } = x;
    ...
    <a href="index.html">
      <button class="HomeBtn">Vuelta a la pagina principal</button>
    </a>
    `;
  }
};
